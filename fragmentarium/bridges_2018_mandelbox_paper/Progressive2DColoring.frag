// Modified from standard Progressive2DColor.frag to include:
// color overlays to show boxfold lines, spherefold bounds, and more
// toggle for changing coloration to grayscale
// toggle for fading coloration
// toggle for flattening coloration of any points that escape under iteration, to a single specified color
...
vec3 color(vec2 c) {
  ...
  if (applyColorOverlay(c, oColor)) {
    ...
    return vec3((0.5 + 0.5*cos(6.0*col))*Fade, 
                (0.5 + 0.5*cos(6.0*col))*Fade, 
                (0.5 + 0.5*cos(6.0*col))*Fade);
  }
  else {
    return vec3((0.5 + 0.5*cos(6.0*col + R))*Fade, 
                (0.5 + 0.5*cos(6.0*col + G))*Fade, 
                (0.5 + 0.5*cos(6.0*col + B))*Fade);
  }
}
